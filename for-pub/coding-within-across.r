  ...
  
    {
          ...
          for (year in years)
          {
            for (month in months)
            {
              ...
              if (month == "March") # remove tweets that have been flagged
              {
                print(nrow(tmp))
...
tiff("~/Dropbox/jree-r-r/for-submission/Images/figure6.tiff", units="in", height = 3, width = 9, res=250)
...
across <- ggplot(melted, aes(x = (reorder(x=variable,as.numeric(rownames(melted)))), y = value, fill = newcode)) + 
  ...
  labs(x="Across Years", y=paste0("Tweets Classified as 'Not Critical'","\n","(precision located above each bar)"))+
...
within <- ggplot(melted_w, aes(x = (reorder(x=variable,as.numeric(rownames(melted_w)))), y = value, fill = newcode)) + 
  ...
  scale_x_discrete(labels = c("2015-Feb", "2015-March", "2015-April"))+
